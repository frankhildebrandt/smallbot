...
3. **Subprocess Manager**: startet die in `settings.yml` konfigurierten Module, verwaltet deren Laufzeit-Verzeichnisse und bindet sie an den Broker an.
...
1. Der Root-Prozess laedt `settings.yml` aus dem Projekt-Root.
2. Der Root-Prozess startet den Broker.
3. Der Subprozess-Manager legt Laufzeitverzeichnisse und Socket-Pfade an.
4. Jedes Modul startet mit:
   - `SERVICE_NAME`
   - `SERVICE_KIND`
   - `MANAGER_SOCKET_PATH`
   - `LISTEN_SOCKET_PATH`
   - `DATA_PATH`
5. Das Modul oeffnet sein Eingangs-Socket und sendet danach eine `register`-Nachricht an den Broker.
6. Der Broker speichert den Service und leitet spaetere Nachrichten anhand von `n` oder `t` weiter.

## Sandbox-Modell

- Im Devcontainer laeuft `bubblewrap` standardmaessig im Modus `auto`.
- `permissions.networking=false` erzwingt einen Netzwerk-isolierten Bubblewrap-Start oder fuehrt zu einem Startfehler, wenn das auf der aktuellen Plattform nicht sicher moeglich ist.
